...
function timelineAppendRow(cycle, row) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    ensureInView(container.parentElement, timelineCellDiv);
  }
}
// for (let i = 1; i < 3; i++) {
...
class PipelineSimulator {
  ...
  step(){
    ...
    // console.log("Hale\n");
    // console.log(knobs.data_knob);
    // console.log(this.pc[1] + "         " + this.pc[2]);
    if(knobs.data_knob   &&  forwardingDepencency(this.pc[1], this.pc[2])){
      // console.log("HALELUJUAH\n");
    ...
    if(this.pc[2] != -1 && this.pc[3] == -1){
      this.pc[3] = this.pc[2];
      this.pc[2] = -1;
      // console.log("Instruction going for memory access"+this.pc[3]);
      // console.log(cycles(decode2(instList[this.pc[3]].split(" ")[1]).op) + '\n');
      // console.log(cycles(decode2(instList[this.pc[3]].split(" ")[1]).op)[3] + '\n');
