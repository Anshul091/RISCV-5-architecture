...
function print_ins(obj)
{
    ...
    {
        let s=obj.op+" "+"x"+obj.rd+" "+"x"+obj.rs1+ " "+"x"+obj.rs2;
        return s;
    }
    ...
    if(op=="beq" || op=="bne" || op=="blt" || op=="bge" || op=="bltu" || op=="bgeu")
    {
...
function cycles(op){
    ...
    operations.fill(1);
    operations[3] = 0;
    if(op==='lw'||op==='lh'||op==='lb'||op==='lhu'||op==='lbu'||op==='sh'||op==='sw'||op==='sb') operations[3]=1;
    if(op==='sb'||op==='sw'||op==='sh'||op[0]==='b')operations[4]=0;
    return operations;
}
...
function register_update(register_file){
  ...
  for(let i = 1; i<32; i++){
    if(type == "Hexadecimal"){
    document.getElementById("register-x"+i).innerText = "0x" + register_file[i].toString(16);
    }
...
class Simulator {
    constructor() {
        ...
        document.getElementById("register").getElementsByClassName("register-container")[0].innerHTML = "";
        for (let i = 0; i < 32; i++) {
        ...
        document.querySelector('#but-hex').addEventListener('click', function(){RFSync(simulator.RF, 'hex');memSync(simulator.MEMORY, 'hex')})
        document.querySelector('#but-dec').addEventListener('click', function(){RFSync(simulator.RF, 'dec');memSync(simulator.MEMORY, 'dec')})
        document.querySelector('#but-udec').addEventListener('click', function(){RFSync(simulator.RF, 'udec');memSync(simulator.MEMORY, 'udec')})
    ...
    memoryAccess() {
        ...
        if(this.OP[0] === 's'){
            ...
            console.log(this.MEMORY[address]+this.MEMORY[address+1]+this.MEMORY[address+2]+this.MEMORY[address+3]);
            let type = document.getElementsByClassName("dropbtn")[0].innerText;
            let numberFormat = 'hex';
            if(type === 'Unsigned Decimal') numberFormat = 'udec';
            if(type === 'Decimal') numberFormat = 'dec';
...
document.querySelector('#thefile').addEventListener('change', async function(e) {
...
function memSync(memory, format){
    ...
    for(let address in memory){
        ...
        if(address>>2 === lastAccess){
            ...
            rowWritten++;
            for(let i=0; i<4; i++) if(memoryLocations[4*rowWritten+i].textContent === '') memoryLocations[4*rowWritten+i].textContent = format==='hex'?'00':'0';
            let memoryAddress = document.querySelectorAll('.memory-address');
            memoryAddress[rowWritten].textContent = '0x'+numberToHexString(address-address%4);
            memoryLocations[4*rowWritten+address%4].textContent = value;
        }
    }
}